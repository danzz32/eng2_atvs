...
def create_cliente(cliente: ClienteCreate, db: Session = Depends(database.get_db)):
...
def list_clientes(db: Session = Depends(database.get_db)):
    repo = ClienteRepository(db)
    return repo.get_all()
...
def update_cliente(cliente_id: int, dados: ClienteCreate, db: Session = Depends(database.get_db)):
    repo = ClienteRepository(db)
